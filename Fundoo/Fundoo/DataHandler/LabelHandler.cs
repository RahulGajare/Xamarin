...
namespace Fundoo.DataHandler
{
    public class LabelHandler
    {
        ...
        {
            await firebaseClient.Child("FundooUsers").Child("UserList").Child(FireBaseThroughAuthentication.GetUid()).Child("Labels").PostAsync<LabelModel>(label);
        }
        ...
        public async Task<List<LabelModel>> GetAllLables()
        {
          ...
          .Child("FundooUsers").Child("UserList").Child(FireBaseThroughAuthentication.GetUid()).Child("Labels")
        ...
        public async Task<bool> SaveLableByKey(LabelModel label, string labelkey)
        {
            try
            {
                await firebaseClient.Child("FundooUsers").Child("UserList").Child(FireBaseThroughAuthentication.GetUid()).Child("Labels").Child(labelkey).PutAsync<LabelModel>(label);
                return true;
            }
            catch (Exception ex)
            {
        ...
        {
            LabelModel lable = await firebaseClient.Child("FundooUsers").Child("UserList").Child(FireBaseThroughAuthentication.GetUid()).Child("Labels").Child(labelKey).OnceSingleAsync<LabelModel>();
            return lable;
        }
        ...
        public async Task<bool> DeleteLableByKey(string labelKey)
        {
            try
            {
                await firebaseClient.Child("FundooUsers").Child("UserList").Child(FireBaseThroughAuthentication.GetUid()).Child("Labels").Child(labelKey).DeleteAsync();
                return true;
            }
            catch (Exception)
            {
